...
def imread(filename):
    ...
    image = preprocess(Image.open(filename)).unsqueeze(0).to("mps")
    return image

def get_activations(files, model, batch_size, dims, mps, verbose, pca_path, gmm_path, output_file):
...
def calculate_activation_statistics(files, model, batch_size, dims, mps, pca_path, gmm_path, output_file):
    verbose = False
    act = get_activations(files, model, batch_size, dims, mps, verbose, pca_path, gmm_path, output_file)
    mu = np.mean(act, axis=0)
    sigma = np.cov(act, rowvar=False)
    return mu, sigma


def _compute_statistics_of_path(path, model, batch_size, dims, mps, pca_path, gmm_path, output_file):
    if path.endswith('.npz'):
        ...
        m, s = calculate_activation_statistics(files, model, batch_size, dims, mps, pca_path, gmm_path, output_file)

    return m, s


def calculate_fid_given_paths(paths, batch_size, mps, dims, pca_path, gmm_path, output_file):
    for p in paths:
    ...
    m1, s1 = _compute_statistics_of_path(paths[0], model, batch_size, dims, mps, pca_path, gmm_path, output_file)
...
if __name__ == '__main__':
    ...
    fid_value = calculate_fid_given_paths(args.path, args.batch_size, device.type != 'cpu', args.dims, args.pca_path, args.gmm_path, args.output_file)
