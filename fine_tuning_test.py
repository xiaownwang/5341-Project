...
def main():
    ...
    parser.add_argument("--n_imgs", type=int, default=100, help="image width, in pixel space")
    ...
    parser.add_argument("--config", type=str, default="", help="path to config which constructs model")
    parser.add_argument("--ckpt", type=str, default="", help="path to checkpoint of model")
    parser.add_argument("--seed", type=int, default=42, help="the seed (for reproducible sampling)")
    parser.add_argument("--precision", type=str, choices=["full", "autocast"], default="autocast",
                        help="evaluate at this precision")
    parser.add_argument("--image_path", type=str, default="", help="evaluate at this precision")
    parser.add_argument("--mask_path", type=str, default="", help="evaluate at this precision")
    parser.add_argument("--reference_path", type=str, default="", help="evaluate at this precision")

    parser.add_argument("--image_folder", type=str, default="dataset/testdata/image", help="folder containing input images")
    parser.add_argument("--mask_folder", type=str, default="dataset/testdata/mask", help="folder containing mask images")
    parser.add_argument("--reference_folder", type=str, default="testdata/reference",
    ...
    for image_file, mask_file, reference_file in zip(image_files, mask_files, reference_files):
        if (not any(image_file.lower().endswith(ext) for ext in valid_image_extensions)) or \
                (not any(mask_file.lower().endswith(ext) for ext in valid_image_extensions)) or \
                (not any(reference_file.lower().endswith(ext) for ext in valid_image_extensions)):
            continue

        image_path = os.path.join(opt.image_folder, image_file)
        mask_path = os.path.join(opt.mask_folder, mask_file)
        reference_path = os.path.join(opt.reference_folder, reference_file)
        ...
        print(f"\n\n######################################################")
        print(f"Processing {image_file}, {mask_file}, {reference_file}")
        print(f"######################################################")

        precision_scope = nullcontext
        with torch.no_grad():
            with precision_scope("mps"):
                with model.ema_scope():
                    img_p = Image.open(image_path).convert("RGB")
                    image_tensor = get_tensor()(img_p).to(torch.float32)
                    image_tensor = image_tensor.unsqueeze(0)

                    ref_p = Image.open(reference_path).convert("RGB").resize((224, 224))
                    ref_tensor = get_tensor_clip()(ref_p).to(torch.float32)
                    ...
                    z_inpaint = z_inpaint.to(torch.float32)
                    test_model_kwargs['inpaint_image'] = z_inpaint

                    height, width = z_inpaint.shape[-2], z_inpaint.shape[-1]
                    mask_resized = F.interpolate(test_model_kwargs['inpaint_mask'], size=(height, width),
                                                 mode='bilinear',
                                                 align_corners=False)
                    test_model_kwargs['inpaint_mask'] = mask_resized
                    test_model_kwargs['inpaint_mask'] = test_model_kwargs['inpaint_mask'].to(torch.float32)
                    ...
                    if not opt.skip_save:
                        for i, x_sample in enumerate(x_checked_image_torch):
                            ...
                            ref_img = F.interpolate(ref_img, size=(opt.H, opt.W), mode='bilinear', align_corners=False)
