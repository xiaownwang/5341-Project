...
class OpenImageDataset(data.Dataset):
    def __init__(self,state,arbitrary_mask_percent=0,**args
        ...
        self.bbox_path_list=[]
        if state == "train":
            dir_name_list=['cartoon']  # ,'1','2','3','4','5','6','7','8','9','a','b','c','d','e','f'
            for dir_name in dir_name_list:
                bbox_dir=os.path.join('dataset/COCO_O','bbox','train_'+dir_name)
            ...
            bbox_dir=os.path.join('dataset/COCO_O','bbox','validation')
            per_dir_file_list=os.listdir(bbox_dir)
            for file_name in per_dir_file_list:
                if file_name not in bad_list:
                    self.bbox_path_list.append(os.path.join(bbox_dir,file_name))
        else:
            bbox_dir=os.path.join('dataset/COCO_O','bbox','test')
            per_dir_file_list=os.listdir(bbox_dir)
            for file_name in per_dir_file_list:
                if file_name not in bad_list:
                    self.bbox_path_list.append(os.path.join(bbox_dir,file_name))
    ...
    def __getitem__(self, index):
        ...
        img_path=os.path.join('dataset/COCO_O/images',dir_name,file_name)
